...
def main():
    ...
    # 断点续训练
    checkpoint_path = '/opt/data/private/lwz/workplace/tutorials/detection/trained_models/model_ct_2nd_last.pt'
    if args.resume:
        print(f'resume training from {checkpoint_path}')
        net = torch.jit.load(checkpoint_path).to(device)
    ...
    coco_metric = COCOMetric(classes=["gun_part","gun","knife","lighter","battery"], iou_list=[0.1], max_detection=[100])
